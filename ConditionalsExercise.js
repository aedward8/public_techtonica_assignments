...
function exercise1(num1) {
  ...
  // I passed two arrays into exercise4 which were the same array but with different names but that still passed as diifer
  // i would of expected this to pass. This might be because the name of the array is also put in consideration
  // I also tried to pass two NaN values into the exercise and that didn't pass either even thought both values were NaN
  // I am not sure why both NaN values did not pass
  ...
  // if ((varA == varB) && (varA || varB != varC)) { answer6=true;}
  // I initially tired to treat the or statement as if it was a english word or but this is comparing varA to 
  // varB!=varC which evalutes as false and varA maybe coerced into being true so true or false encompases everything 
  // and it will always pass true
  ...
  //
  // I had to use google to remeber to use the remainder operator % as part of the if statement for checking if a values
  // was odd or even
  // For Exercise 7, I was not familiar with switch conditional statements so I had to use w3schools.com to understand what these were
